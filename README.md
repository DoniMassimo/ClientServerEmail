...
1. **Mail Server**
   - Gestisce le caselle di posta elettronica degli utenti registrati

2. **Mail Client**
   - Permette agli utenti di leggere e inviare email

---

## ⚙️ ARCHITETTURA GENERALE

- Entrambe le applicazioni sono JavaFX basate su pattern **MVC**
- Progetti separati in IntelliJ IDEA
- Nessuna comunicazione diretta tra View e Model
- Comunicazione solo tramite Controller o pattern Observer/Observable
- ❌ Vietato usare `Observer.java` e `Observable.java` deprecati
- Usare JavaFX Properties / ObservableList

---

## 🌐 COMUNICAZIONE CLIENT-SERVER

- Client e Server in **JVM separate**
- Comunicazione solo tramite **socket Java**
- Solo scambio di **dati testuali**
- Connessione non persistente (stile HTTP)
- Il client apre la connessione solo quando serve

---

## ⚡ REQUISITI GENERALI

- Parallelizzare le operazioni non sequenziali
- Gestire accesso concorrente alle risorse
- Sistema scalabile (molti utenti, mailbox grandi)
- Non trasferire intere mailbox tra client e server
...
- L’utente inserisce la propria email all’avvio
- L’email è identificatore dell’utente
- Validazione sintattica con **Regex**
- Se errata → reinserimento obbligatorio
- Il server verifica l’esistenza dell’account
...
- inbox (messaggi ricevuti non cancellati)
...
La GUI gestisce SOLO la inbox:

### 📩 Funzionalità

- Visualizzare lista messaggi
- Selezionare e leggere messaggi
- Visualizzare dettagli messaggi
- Cancellare messaggi

### ✉️ Invio email

- Creazione messaggi
- Invio a uno o più destinatari
- Reply (solo mittente)
- Reply-All (tutti i destinatari + mittente)
- Forward a più destinatari

### ✔️ Validazioni

- Controllo sintattico email (Regex)
- Verifica esistenza destinatari lato server
...
- Inbox aggiornata automaticamente (no refresh manuale)
- Notifica arrivo nuovi messaggi
- Interfaccia:
  - semplice
  - chiara
  - efficiente (pochi click)
...
- Salvataggio su file:
  ...
  - bin
  - JSON (opzionale)
- ❌ Vietato usare database

---

## 📦 STRUTTURA DATI

Ogni mailbox contiene:

- email account (es. user@domain.com)
- lista messaggi
...
Mostra un **log eventi**:

- apertura/chiusura connessione
- invio messaggi
- errori di consegna
- errori server-client

❌ NON loggare azioni del client (click UI ecc.)

---

## 👥 ACCOUNT

- numero fisso di utenti (es. 3)
- utenti preconfigurati
- nessuna registrazione da client

---

# 🔄 COMUNICAZIONE

## 📌 SERVER RESPONSABILITÀ

- Verifica esistenza email
- Validazione destinatari
- Invio errori al client

---

## ⚠️ GESTIONE ERRORI

- Il client NON deve crashare se il server è offline
- Deve gestire:
  - disconnessione
...
- Non trasferire intere mailbox
- Server invia solo nuovi messaggi non ancora consegnati
